...
function MembershipDetailContent({ membershipId }: { membershipId: string }) {
  ...
  const activityLogs = useActivityLogs(membershipId);
    ...
    return (
      <div className="p-8 max-w-7xl mx-auto space-y-4">
        ...
        <div className="rounded-2xl border border-border bg-card p-12 text-center text-muted-foreground">
          <p className="text-sm font-semibold">Membership record not found.</p>
        </div>
      </div>
    );
  }

  const currentRole = roles.find((r) => r.id === membership.roleId);
  ...
  const handleToggleStatus = () => {
    ...
    updateStatusMutation.mutate({ memberId: membership.id, status: newStatus });
  };
      ...
      <div className="rounded-2xl border border-border bg-card p-6 shadow-xs text-card-foreground space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {membership.role} • {membership.department} Department • {membership.membershipType}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {isSuspended ? (
                <>
                  <UserCheck className="h-4 w-4" /> Reactivate Membership
                </>
              ) : (
                <>
                  <UserX className="h-4 w-4" /> Suspend Membership
                </>
              )}
            </button>
            <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-3 border-t border-border text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            ...
            <span className="text-foreground font-medium">{membership.email}</span>
          </div>
          <div className="flex items-center gap-2">
            <Phone className="h-4 w-4 text-muted-foreground" />
            <span className="text-foreground font-medium">{membership.phone}</span>
          </div>
          <div className="flex items-center gap-2">
            <Calendar className="h-4 w-4 text-muted-foreground" />
            <span>Joined Club: {membership.joinedDate}</span>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="rounded-2xl border border-border bg-card p-6 shadow-xs space-y-4">
              ...
              <div className="pt-3 border-t border-border">
                <Link
                  href={`/dashboard/roles/${membership.roleId}`}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-2xl border border-border bg-card p-4 shadow-xs">
            <div>
              <h2 className="text-sm font-bold text-foreground">
                Individual Permission Overrides for {membership.name}
              </h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                Grant custom permissions or revoke specific access rights directly on this membership, independent of their role.
              </p>
            </div>
            <button
              type="button"
...
export default function MembershipDetailPage({ params }: MembershipDetailPageProps) {
  ...
  return (
    <Suspense fallback={<div className="p-8 text-xs text-muted-foreground">Loading membership details...</div>}>
      <MembershipDetailContent membershipId={resolvedParams.membershipId} />
    </Suspense>
  );
}
